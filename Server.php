  ...
  class Server {
    ...
    public static function addServer($servername, $hostname, $folder_id = null) {
			try {
					...
					if ( $folder_id != null ) {
					if( !Server::moveServer($server_id, $folder_id) )
    ...
    public static function moveServer($server_id, $folder_id) {
      ...
      if( !Server::serverRunCommandName($server_id, CommandNames::RestartRequired, $restart_required, $error) ) {
        return false;
      }
      if( !Server::serverRunCommandName($server_id, CommandNames::RebootGet, $sheduled_restart, $error) ) {
        return false;
      }
      if( !Server::serverRunCommandName($server_id, CommandNames::Distribution, $distribution, $error) ) {
        return false;
      }
      if( !Server::serverRunCommandName($server_id, CommandNames::DistributionVersion, $distribution_version, $error) ) {
        return false;
      }
      if( !Server::serverRunCommandName($server_id, CommandNames::ListUpdates, $update_list_str, $error) ) {
        return false;
      }
      ...
      if( ! Server::serverGetEOL($server_id, $EOL) ) {
        $EOL = null;
      }
      ...
      if( ! Config::updateServerInfo($server_id, $uptime, $restart_required,
    ...
    public static function deleteServer($server_id) {
      ...
      if( !Server::getServerDistribution($server_id, $distri, $distri_version) ) {
        if( !Config::serverDetectDistribution($server_id, $distri, $distri_version, $error2) ) {
          $error = "Can't detect distribution: " . $error2;
          return false;
        }
      }
      if( !Config::getDistributionCommand($distri, $distri_version, $commandname, $cmd) ) {
        $error = "No command for $commandname specified for distribution " . $distri . " " . $distri_version;
        return false;
      }
      return Server::serverRunCommand($server_id, $cmd, $command_ret, $error);
    }
    ...
    public static function serverRunCommand($server_id, $command, &$command_ret, &$error) {
      ...
      $ret =  Server::sshRunCommand($ssh_hostname, $ssh_port, $ssh_username, $ssh_key,
    ...
    public static function getServerDistribution($server_id, &$distri, &$distri_version) {
      ...
      if( ! Server::getServerDistribution($server_id, $distri, $distri_version) )
        return false;
      return Server::getEOLByDistribution($distri, $distri_version, $EOL);
		}
      ...
      try {
        ...
        if( !Config::getDistributionCommand($distri, $distri_version, CommandNames::RebootSet, $cmd) ) {
          $error = "No command for $commandname specified for distribution " . $distri . " " . $distri_version;
          return false;
        }
        $dt = new DateTime();
        ...
        $cmd = str_replace('${mm}', $mm, $cmd);
        if( !Server::serverRunCommand($server_id, $cmd, $output, $error) ) {
          return false;
        }
        if( !Server::serverRunCommandName($server_id, CommandNames::RebootGet, $sheduled_restart, $error) ) {
          return false;
    ...
    public static function delSheduleReboot($server_id, &$serverinfo) {
      try {
        ...
        if( !Server::serverRunCommandName($server_id, CommandNames::RebootDel, $output, $error) ) {
          return false;
        }
