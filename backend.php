<?php
  ...
  switch( $fct_name ) {
    case "get_folders":
      if(!RbacCommands::enforcePermissions(7,1))
        echo json_encode( array('success' => false, 'message' => "You don't have the permission to access this ressource"));
        break;
      else
        ...
        break;
    case "get_servers":
      if(!RbacCommands::enforcePermissions("Watcher",$user_id))
        echo json_encode( array('success' => false, 'message' => "You don't have the permission to access this ressource"));
        break;
      else
        ...
        break;
    case "add_server":
      if(!RbacCommands::enforcePermissions("Configurator",$user_id))
        echo json_encode( array('success' => false, 'message' => "You don't have the permission to access this ressource"));
        break;
      else
    ...
    case "add_package_important":
      if(!RbacCommands::enforcePermissions("Admin",$user_id))
        echo json_encode( array('success' => false, 'message' => "You don't have the permission to access this ressource"));
        break;
      else
        $server_id = $fct_data['server_id'];
    ...
    case "inventory_server_from_list":
      if(!RbacCommands::enforcePermissions("Inventorist",$user_id))
        echo json_encode( array('success' => false, 'message' => "You don't have the permission to access this ressource"));
        break;
      else
        $server_id = $fct_data["server_id"];
    ...
    case "get_package_info":
      if(!RbacCommands::enforcePermissions("Watcher",$user_id))
        ...
        break;
    case "update_package":
      if(!RbacCommands::enforcePermissions("Updater",$user_id))
        echo json_encode( array('success' => false, 'message' => "You don't have the permission to access this ressource"));
        break;
      else
        $update_id = $fct_data["update_id"];
    ...
    case "shedule_reboot_add_list":
      if(!RbacCommands::enforcePermissions("Reboot",$user_id))
        echo json_encode( array('success' => false, 'message' => "You don't have the permission to access this ressource"));
        break;
      else
        $server_id = $fct_data['server_id'];
    ...
    case "get_users_with_roles":
      if(!RbacCommands::enforcePermissions("RBAC",$user_id))
        echo json_encode( array('success' => false, 'message' => "You don't have the permission to access this ressource"));
        break;
      else
        if (RbacCommands::getUsersWithRoles($users))
          echo json_encode(array('sucess' => true, 'users' => $users) );
        else
          echo json_encode(array('success' => false) );
        break;
    case "get_roles":
      if(!RbacCommands::enforcePermissions("RBAC",$user_id))
        ...
        if ( RbacCommands::getRoles($roles))
          echo json_encode( array('success' => true, 'roles' => $roles) );
        else
          echo json_encode( array('success' => false, 'message' => "Error while getting roles") );
        break;
    case "get_users":
      if(!RbacCommands::enforcePermissions("$RBAC",$user_id))
        echo json_encode( array('success' => false, 'message' => "You don't have the permission to access this ressource"));
        ...
        if ( RbacCommands::get_users())
          echo json_encode( array('success' => true) );
        else
          echo json_encode( array('success' => false, 'message' => "Error while getting users") );
        break;
    case "add_user_to_role":
       if(!RbacCommands::enforcePermissions("RBAC",$user_id))
        ...
        $role_id = $fct_data['role_id'];
        $user_id = $fct_data['user_id'];
        if ( RbacCommands::add_user_to_role($role_id, $user_id) )
          echo json_encode( array('success' => true) );
        else
          echo json_encode( array('success' => false, 'error' => "Error while removing user from role") );
        break;
    case "remove_user_from_role":
      if(!RbacCommands::enforcePermissions("$RBAC",$user_id))
        ...
        $role_id = $fct_data['role_id'];
        $user_id = $fct_data['user_id'];
        if ( RbacCommands::remove_user_from_role($role_id, $user_id) )
          echo json_encode( array('success' => true) );
        else
          echo json_encode( array('success' => false, 'error' => "Error while removing user from role") );
        break;
